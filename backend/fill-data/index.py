'''
Business: Заполняет эпизоды просмотрами, лайками и комментариями
...
def generate_comment(season: int) -> Tuple[str, str, int]:
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        for ep_id, title, season, episode in episodes:
            ...
            cur.execute(f"UPDATE episodes SET views={views}, likes={likes} WHERE id={ep_id}")
            
            num_comments = random.randint(30, 100)
            
            for _ in range(num_comments):
                name, text, rating = generate_comment(season)
