          ...
          <div className="space-y-6">
            ...
            {allEpisodes.length > 0 && (
              <Card className="bg-gray-800/50 border-purple-500/30 p-6">
                ...
                <div className="space-y-2">
                  {Array.from(new Set(allEpisodes.map(ep => ep.season)))
                    .sort((a, b) => a - b)
                    .map(season => {
                      const seasonEpisodes = allEpisodes.filter(ep => ep.season === season);
                      const isCurrentSeason = episode && episode.season === season;
                      return (
                        <div
                          key={season}
                          className={`p-3 rounded-lg ${
                            isCurrentSeason ? 'bg-purple-500/20 border-l-4 border-purple-400' : 'bg-gray-700/30'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <span className={`font-semibold ${
                              isCurrentSeason ? 'text-purple-400' : 'text-gray-300'
                            }`}>
                              Сезон {season}
                            </span>
                            <span className="text-sm text-gray-400">
                              {seasonEpisodes.length} эп.
                            </span>
                          </div>
                        </div>
                      );
